...
# Access via POST and is exempt from CSRF verification.
@require_POST
@csrf_exempt
def webhook(request):
    ...
    # Get the payload from the incoming request and the Stripe signature.
    payload = request.body
    sig_header = request.META['HTTP_STRIPE_SIGNATURE']
    event = None

    try:
        # Construct the event using the library function to validate.
    ...
    # Fetch the corresponding event handler from the map.
